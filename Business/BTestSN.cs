...
namespace Business
{
    public class BTestSN
    {
        ...
        public int InsertOrUpdateProductType(int? ProductTypeID, string ProductType, Boolean IsActive, int UserID)
        {
            TraceLogger.Log(DateTime.Now);
            int success = 0;
            DbParameter ProductTypeIDP = provider.CreateParameter("ProductTypeD", ProductTypeID, DbType.Int32);
        ...
        public int InsertOrUpdateProduct(int? ProductID, string Product, Boolean IsActive, int UserID)
        {
            TraceLogger.Log(DateTime.Now);
            int success = 0;
            DbParameter ProductIDP = provider.CreateParameter("ProductTypeD", ProductID, DbType.Int32);
            DbParameter ProductP = provider.CreateParameter("ProductType", Product, DbType.String);
            DbParameter IsActiveP = provider.CreateParameter("IsActive", IsActive, DbType.Boolean);
            DbParameter UserIDP = provider.CreateParameter("UserID", UserID, DbType.Int32);
