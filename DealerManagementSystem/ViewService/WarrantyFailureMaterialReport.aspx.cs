...
{
    public partial class WarrantyFailureMaterialReport : BasePage
    {
        public override SubModule SubModuleName { get { return SubModule.ViewService_WarrantyFailureMaterialReport; } }
        ...
        void fillClaim()
        {
            try
            {
                ...
                DateTime? DCDateF = null;
                DateTime? DCDateT = null;

                if (!string.IsNullOrEmpty(txtDCDateF.Text))
                {
                    DCDateF = Convert.ToDateTime(txtDCDateF.Text);
                }
                if (!string.IsNullOrEmpty(txtDCDateT.Text))
                {
                    DCDateT = Convert.ToDateTime(txtDCDateT.Text);
                }

                //  CategoryID = ddlCategory.SelectedValue == "0" ? (int?)null : Convert.ToInt32(ddlCategory.SelectedValue);
                List<PDMS_WarrantyInvoiceHeader> SOIs = new BDMS_WarrantyFailureMaterial().GetFailedMaterialReturn(txtICServiceTicket.Text.Trim(), ICTicketDateF, ICTicketDateT, txtClaimID.Text.Trim(), ClaimDateF
                    , ClaimDateT, DealerCode, StatusID, txtDCNumber.Text.Trim(), DCDateF, DCDateT, PSession.User.UserID);
        ...
        protected void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            dt.Columns.Add("Material Return Status");
            dt.Columns.Add("DC Number");
            dt.Columns.Add("Delivery Challan Date");
            dt.Columns.Add("Transporter Name");
            dt.Columns.Add("Docket Details");
            dt.Columns.Add("date of confirm");
                ...
                foreach (PDMS_WarrantyInvoiceItem Item in M.InvoiceItems)
                {
                    dt.Rows.Add(
                       ...
                       , M.Approved2By.ContactName
                        ...
                        , M.AcInvoiceNumber
                        , Item.AnnexureNumber
                        , Item.WarrantyMaterialReturnStatus.WarrantyMaterialReturnStatus
                        , Item.FailureMaterial.DeliveryChallanNumber
                          , Item.FailureMaterial.DeliveryChallanDate
                          , Item.FailureMaterial.TransporterName
                          , Item.FailureMaterial.DocketDetails
                          , Item.FailureMaterial.FailureMaterialItem.AcknowledgedOn
                          , Item.FailureMaterial.PackingDetails
                        );
                }
            }
            new BXcel().ExporttoExcel(dt, "Failure Material");
        }
        ...
        void FillStatus()
        {
            List<PDMS_WarrantyMaterialReturnStatus> Status = new BDMS_WarrantyFailureMaterial().GetWarrantyMaterialReturnStatus(null, null);
            ddlStatus.DataTextField = "WarrantyMaterialReturnStatus";
            ddlStatus.DataValueField = "WarrantyMaterialReturnStatusID";
        ...
        protected void btnExportExcelForSAP_Click(object sender, EventArgs e)
        {
            ...
            foreach (PDMS_WarrantyInvoiceHeader M in SDMS_WarrantyClaimHeader)
            {
                foreach (PDMS_WarrantyInvoiceItem Item in M.InvoiceItems)
                {
                    if (!Item.FailureMaterial.FailureMaterialItem.IsCanceled)
                    {
                        dt.Rows.Add(
