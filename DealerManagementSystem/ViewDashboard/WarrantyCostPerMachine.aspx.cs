...
namespace DealerManagementSystem.ViewDashboard
{
    public partial class WarrantyCostPerMachine : BasePage
    {
        ...
        {
            // Handle the event
            //  Label1.Text = "Button in User Control clicked!";
            ddlmModel.Fill("DivisionDescription", "DivisionID", new BDMS_Master().GetDivision(null, null));
        }
        ...
        public static List<object> RegionEastChart(string DateFrom, string DateTo, string AsOnDate)
        {
            ...
            DataTable dt = ((DataSet)new BDMS_WarrantyClaim().DAGetQuality(DateFrom, DateTo, AsOnDate, Dealer, Region, ServiceType, Division, Model)).Tables[0];
        ...
        protected void BtnLineChartData_Click(object sender, EventArgs e)
        {
            ...
            string ServiceType = (string)HttpContext.Current.Session["ServiceType"];
            string Model = (string)HttpContext.Current.Session["Model"];
            DataTable dt = ((DataSet)new BDMS_WarrantyClaim().DAGetQuality(txtMfgDateFrom.Text.Trim(), txtMfgDateTo.Text.Trim(), txtAsOnDate.Text.Trim(), Dealer, Region, ServiceType, Division, Model)).Tables[0];
            new BXcel().ExporttoExcel(dt, "Warranty LineChart Data");
        }
        ...
        protected void BtnDetailData_Click(object sender, EventArgs e)
        {
            ...
            string ServiceType = (string)HttpContext.Current.Session["ServiceType"];
            string Model = (string)HttpContext.Current.Session["Model"];
            DataTable dt = ((DataSet)new BDMS_WarrantyClaim().DAGetQuality(txtMfgDateFrom.Text.Trim(), txtMfgDateTo.Text.Trim(), txtAsOnDate.Text.Trim(), Dealer, Region, ServiceType, Division, Model, 1)).Tables[0];
            new BXcel().ExporttoExcel(dt, "Warranty Cast Data");
        }
    }
}