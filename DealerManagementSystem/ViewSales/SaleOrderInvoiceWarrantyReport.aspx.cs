...
{
    public partial class SaleOrderInvoiceWarrantyReport : BasePage
    {
        public override SubModule SubModuleName { get { return SubModule.ViewSales_SaleOrderInvoiceWarrantyReport; } }
        public List<PDMS_SalesInvoice> SDMS_SalesInvoice
        {
            get
            {
                if (Session["DMS_SaleOrderInvoiceWarrantyReport"] == null)
                {
                    Session["DMS_SaleOrderInvoiceWarrantyReport"] = new List<PDMS_SalesInvoice>();
                }
                return (List<PDMS_SalesInvoice>)Session["DMS_SaleOrderInvoiceWarrantyReport"];
            }
            set
            {
                Session["DMS_SaleOrderInvoiceWarrantyReport"] = value;
            }
        }
        ...
        {
            Page.ClientScript.RegisterStartupScript(this.GetType(), "Script1", "<script type='text/javascript'>SetScreenTitle('Sales » Warranty Invoice');</script>");
        ...
        void fillSalesInvoice()
        {
            ...
            try
            {
                ...
                if (rbDetail.Checked)
                {
                    SOIs = new BDMS_SalesOrder().GetSaleOrderInvoiceWarrantyReport(DealerID, CustomerCode, InvoiceNumber, InvoiceDateF, InvoiceDateT, SaleOrderInvoiceStatusID, txtMaterial.Text.Trim());
                    gv = gvICTickets;
                }
                if (rbDealerMaterial.Checked)
                {

                    SOIs = new BDMS_SalesOrder().GetSaleOrderInvoiceWarrantyDealerAndMaterialWise(DealerID, CustomerCode, InvoiceNumber, InvoiceDateF, InvoiceDateT, SaleOrderInvoiceStatusID, txtMaterial.Text.Trim());
                    gv = gvDM;
                }
                if (rbDealerCustomerMaterial.Checked)
                {
                    SOIs = new BDMS_SalesOrder().GetSaleOrderInvoiceWarrantyDealerCustomerAndMaterialWise(DealerID, CustomerCode, InvoiceNumber, InvoiceDateF, InvoiceDateT, SaleOrderInvoiceStatusID, txtMaterial.Text.Trim());
                    gv = gvDCM;
                }
                gv.Visible = true;
