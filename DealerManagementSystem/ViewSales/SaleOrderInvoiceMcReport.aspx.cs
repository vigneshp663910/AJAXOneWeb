...
{
    public partial class SaleOrderInvoiceMcReport : BasePage
    {
        public override SubModule SubModuleName { get { return SubModule.ViewSales_SaleOrderInvoiceMcReport; } }
        public List<PDMS_SalesInvoice> SDMS_SalesInvoice
        {
            get
            {
                ...
                {
                    Session["DMS_SaleOrderInvoiceMcReport"] = new List<PDMS_SalesInvoice>();
                }
                return (List<PDMS_SalesInvoice>)Session["DMS_SaleOrderInvoiceMcReport"];
            }
            set
            {
        ...
        {
            Page.ClientScript.RegisterStartupScript(this.GetType(), "Script1", "<script type='text/javascript'>SetScreenTitle('Sales » Machine Invoice');</script>");
        ...
        void fillSalesInvoice()
        {
            ...
            try
            {
                ...
                if (rbDetail.Checked)
                {
                    SOIs = new BDMS_SalesOrder().GetSaleOrderInvoiceMcReport(DealerID, CustomerCode, InvoiceNumber, InvoiceDateF, InvoiceDateT, SaleOrderInvoiceStatusID, txtMaterial.Text.Trim());
                    gv = gvICTickets;
                }
                if (rbDealerMaterial.Checked)
                {

                    SOIs = new BDMS_SalesOrder().GetSaleOrderInvoiceMcDealerAndMaterialWise(DealerID, CustomerCode, InvoiceNumber, InvoiceDateF, InvoiceDateT, SaleOrderInvoiceStatusID, txtMaterial.Text.Trim());
                    gv = gvDM;
                }
                if (rbDealerCustomerMaterial.Checked)
                {
                    SOIs = new BDMS_SalesOrder().GetSaleOrderInvoiceMcDealerCustomerAndMaterialWise(DealerID, CustomerCode, InvoiceNumber, InvoiceDateF, InvoiceDateT, SaleOrderInvoiceStatusID, txtMaterial.Text.Trim());
                    gv = gvDCM;
                }
                gv.Visible = true;
